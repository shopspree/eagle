class Api::V1::SubcategoriesController < Api::V1::BaseController
  ...
  def index
    @subcategories = if params[:category_id]
                       Category.find(params[:category_id]).subcategories.where(context_id: current_actor.context_id).order("posts_count DESC") unless current_actor.nil?
                     else
  ...
  def create
    @subcategory = Category.find(params[:category_id]).subcategories.new(params[:Subcategory])
